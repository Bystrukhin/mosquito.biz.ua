...
@section('content')
    ...
    @foreach($comments as $comment)
        ...
        <form action="{{ route('admin.editComment', ['comment_id' => $comment->id, 'article_id'=>$comment->article_id]) }}">
            <button class="comment-btn" type="submit">Edit Comment</button>
        </form>
        <form action="{{ route('admin.deleteComment', ['comment_id' => $comment->id, 'article_id'=>$comment->article_id]) }}">
